...
class SelfEnv(gym.Env):    # The wrapper encapsulates the Duckietown env
    ...
    def step(self, action):
        if action == 0: # forward
            new_action = np.array([0.23, 0]) * 2
        elif action == 1: # right
            new_action = np.array([0.09, -0.39]) * 2 
        elif action == 2: # left
            new_action = np.array([0.12, 0.38]) * 2
...
#model = DQN("CnnPolicy", env, buffer_size=50000, verbose=1, tensorboard_log=orig_log_path, exploration_fraction=0.05, learning_rate=0.00005)
